...
This paper discusses some experiences and workflows from the [INEL project](https://github.com/nikopartanen/syktyvkar2017/blob/master/partanen2017a.pdf) (https://inel.corpora.uni-hamburg.de) (*"Grammatiken, Korpora und Sprachtechnologie für Indigene nordeurasische Sprachen"*). 
...
Digital preservation is necessary for the preservation of written cultural heritage, and there are already good standards for the digitalization of books and manuscripts.
...
Opening these kinds of datasets is still in its early phases, and many related practices are largely underdeveloped.
For example, in Scandinavia, libraries have recently begun to offer digitalized materials for public use, although the focus has been mainly on publications before 1910 [@paakkonenEtAl2016a] (http://www.dlib.org/dlib/july16/paakkonen/07paakkonen.html).
Within language documentation projects, the concept of open data is barely discussed, and the most of the data seems to be behind variously restricted licenses or, alternatively, with limitations to only academic use.
...
This is very unfortunate, since incorrectly structured and licensed materials will require considerable future effort and negotiation before they can be taken into active research use.

In order to do enable better usability, content must be accessible. In the case of written documents, this refers mainly to text data.
Optical Character Recognition (henceforth OCR) tools are irreplaceable in efforts to bring printed and handwritten materials into further use.
...
During the last years, there have been multiple very important and large digitalization projects run by both academic institutions and libraries, often in cooperation with one another.
...
Others, namely [Fenno-Ugrica](https://fennougrica.kansalliskirjasto.fi/) (https://fennougrica.kansalliskirjasto.fi/) and the [Komi Kyv](http://komikyv.ru/) (http://komikyv.ru) website maintained in Syktyvkar, are good examples of the systematic digitalization of written resources.
...
In the INEL project, we have been working with the digitalization of Selkup, Dolgan and Kamas materials.
...
Alongside Komi-Permyak, Tundra Nenets also likely falls into the category where the number of available publications is particularly high.
Indeed, with smaller languages, it is likely that the recorded and transcribed materials exceed the printed ones in size; generally speaking, the ratio of these two can be used as one way to estimate the usage situation or vitality of the language.
...
Daniel Jettka is currently implementing a graph database as a solution for storing interlinked data, and this looks like a good solution for the complicated data model inherent to our data. 
For example, in the case of some Selkup transcriptions, the recordings available are apparently different versions from those on which the existing transcriptions are based.
...
This is also a very particular case for OCR tools, since the scripts used are often not included in available OCR models, the number of diacritics can make the scripts extremely complex, and the fonts used may be highly customized to suit specific publications.
...
In many cases the copyright questions also tend to get very complex when it comes to legacy materials, since the rights of the speakers, original researchers, archives and publishers may all interact in many ways.
...
# The basics of OCR workflow

Most of the time, the first step after digitalization is to find some solutions for performing OCR on the text.
To take this further, there are several tasks which must be undertaken before the data is actually ready to be used by a general audience or by researchers.
...
Although we can often rely on command line tools, there are tasks for which a graphical user interface is particularly unsuitable.
As mentioned, one of these is the post-processing of OCR results.
...
This poses the question of so many projects have ended up deciding to develop new tools for post-correction.
...
The export format poses a more serious problem, as it is connected to post-processing and reuse.
The following attributes would ideally be present in the output format:
...
Tesseract, and its derivations such as [OCRicola](https://sourceforge.net/projects/ocricola), are entirely open source and highly trainable, but they offer only the very core functionality of OCR recognition and thereby must be used in conjunction with different tools, and, at least in INEL, we have not yet found a good combination of tools that could offer a highly reliable and reproduceable workflow.
...
I will cover this more in detail below, but the problem within thism is that the models trained in Abbyy Desktop cannot be used in Abbyy Engine, which would have needed XML export.
...
However, with rare scripts and endangered languages, the current language models are highly insufficient and some additional training is necessary in order to achieve a satisfactory output.
...
In Revizor, the information is usually kept, but especially at the line boundaries the program seems to lose its information about which words were originally on which line.
...
In INEL, we are currently working through Abbyy Desktop plain text file and HTML exports, which both do a good job of keeping the paragraph structure and, with some manual maintenance, also page numbering.
With the texts we have worked on, it has been possible to distinguish page numbers as more-or-less sequentially growing numbers that are the only items on the line, and chapter numbers as those that are on the same line as the title or the first line of the text; in any case, not alone.
However, these kinds of definitions are not very generic and also require continuous manual adjustation.
...
## The basics of OCR model training

OCR model training can be carried out at different levels, and the programs available, Abbyy and Tesseract, employ opposite strategies.
...
However, if the model has exact information about all possible word forms, the recognition quality can possibly be improved even further.
...
This makes it possible to use another Cyrillic model, such as Russian, as a backbone and build upon that by adding the special characters needed for other languages.
...
I have often encountered the idea that page position information is not necessary.
...
The same goes for paragraph information, since a paragraph is basically a structure in text that is closely related to the organization of line indent, which, when known, makes paragraph detection rather reliable.
...
For example, when working with texts like the Bible or Koran, the original verse structure is usually enough to distinguish different elements, and in many instances the sameness of the text in each version is very highly enforced.
Some texts resulting from OCR, for example elicited sentences or structured questionnaires, follow essentially the same model where each utterance can be identified as a part of these closed sets.
...
What I would mainly like to emphasize here, is that it is somewhat questionable how well the current tools actually store information about what exactly has been done at which point.
...
There are some experiences demonstrating that a text resulting from OCR can already be corrected to some degree using simple ngram-based post-processing [@arkhangelskiyEtAl2016a].
This is certainly true, especially with data that is relatively well recognized, although the worst-recognized parts should probably be approached by redoing the OCR [@kettunenEtAl2016b].
...
Needless to say, this can already produce a very large corpus, as is demonstrated by the example of our colleagues in Syktyvkar [INSERT REFERENCE HERE!!!!!!!!!!!!!!].
...
In the latter case, the document on which the OCR has been carried out is the actual primary data source, or the closest representation of that which we can have.
Thereby there must be a relatively close connection between the original text document and the later XML file.
...
First of all, there is the need to transform highly idiosynchronic writing systems into the same, ideally phoneme-level representation.
...
Therefore, applying different tools to this end is highly desiderable.
...
Instead of variables such as publishing time and place, there is the obvious need to know writer's birth time and birth place.
These already tell quite a bit about the possible native dialect of the writer, which again can be one way to explain idiosynhcronic features of individual texts.
...
In the Fenno-Ugrica project, one of the main ideas has clearly been to collect resources that exist in several languages spoken in Russia.
The result is that there are now publications such as Four Battles, which are translated to nine different Uralic languages.
A cursory Google search reveals that translations must exist also in various Turkic languages.
When the books are compared, the texts reveal themselves to be immediately useful, since they share exactly the same structure, often almost on the sentence level with only small deviations.
...
Recently also a corpus of movie subtitles has been compiled ([ParTy](http://www.natalialevshina.com/corpus.html) by Natalia Levshina), but since that data comes with matching timelines, the task of alignment is very different.

However, there are also resources that are parallel in less obvious ways.
There are historical events that have been covered in every newspaper in the Soviet Union and beyond.
The data does not result in parallel sentences as such, but still those texts are thematically linked in very intriguing ways.
...
Even more abstract parallels can be drawn between spoken and written resources, since there are many stories and narratives that have been recorded in different retellings and also published multiple times.
...
This is why many large text corpora exist now for pre-1920s data, and as the years go by, this year will gradually keep advancing.
That said, it creates a situation where almost none of the presenters in this conference will live to see the currently published texts enter Public Domain.
