...
Digital preservation has many advantages for the preservation of written cultural heritage, although it is not a necessity in itself, as the paper has also proven as a long-term medium.
...
After digitalization the next logical step after preservation is to bring these materials into common and scientific use.
...
In the case of written documents, this refers mainly to text data.
...
During the last years, there have been multiple large digitalization projects run by both academic institutions and libraries, often in cooperation with one another.
...
This is also a very particular case for OCR tools, since the scripts used are often not included in available OCR models, the number of diacritics (possible added by hand) can make the scripts extremely complex, and the fonts used may be highly customized<!-- to suit specific publications-->.
...
<!-- In many cases--> The copyright questions also get very complex when it comes to legacy materials, since the rights of the speakers, original researchers, archives and publishers may all interact in many ways, and may be next to impossible to reconstruct.
...
Although we can often rely on command line tools, there are tasks for which a graphical user interface is particularly suitable.
...
Tesseract, and its derivations such as [OCRicola](https://sourceforge.net/projects/ocricola), are open source and highly trainable, but they offer only the core functionality of OCR recognition and thereby must be used in conjunction with other tools, and, at least in INEL, we have not yet found a satisfactory combination<!-- of tools that could offer a highly reliable and reproduceable workflow-->.
...
I will cover this more in detail below, but the problem is that the models trained in Abbyy Desktop cannot be used in Abbyy Engine, which would have needed XML export.
...
In INEL, we are currently working through Abbyy Desktop plain text file and HTML exports, which both do a good job of keeping the paragraph structure and page numbering.
...
Detecting it reliably from unstructured lines is, on the other hand, rather hopeless.
...
What I would mainly like to emphasize here, is that it is somewhat questionable how well the current tools actually store information about the entire workflow.
...
There are also resources that are parallel in less obvious ways.
...
Even more abstract parallels can be drawn between spoken and written resources, since there are many stories and narratives that have been recorded in different retellings and published multiple times.
